...
class WCB( inkex.Effect ):
			...
			if self.options.tab == '"splash"': 
				...
				if self.serialPort is None:
					...
					if self.resumeMode:
						...
						ebb_motion.sendDisableMotors(self.serialPort)	#diaable motor
	...
	def setupCommand( self ):
		...
		if self.options.setupType == "align-mode":
			...
			ebb_serial.command( self.serialPort,  'SE,1,30\r\n' )
...
# 		spewSegmentDebugData = True
	...
	def penUp( self ):
		...
		if ( not self.resumeMode) and (not self.bPenIsUp):	# skip if pen is already up, or if we're resuming.
			...
			if (vTime < 0): #Do not allow negative delay times
				vTime = 0	
			if self.options.setupType == "laser-power-on" or self.options.setupType == "laser-power-off":
					self.laserPowerOff()
			else :
	...
	def laserPowerOff( self ):
		...
		ebb_serial.command( self.serialPort,  'SE,0,'+ str(power) +'\r\n' )
	...
	def laserPowerOn( self ):
		...
		if power == 0:
			...
			ebb_serial.command( self.serialPort,  'SE,1,'+ str(power) +'\r\n' )
